...
async def test_loop_notify_stop_when_connection_closed_old_aiopg(asyncpg_connector):
    # We want to make sure that the when the connection is closed, the loop end.
    event = asyncio.Event()
    await asyncpg_connector.open_async()
    async with asyncpg_connector._pool.acquire() as connection:
        coro = asyncpg_connector._loop_notify(event=event, connection=connection)
...
async def test_loop_notify_stop_when_connection_closed(asyncpg_connector):
    # We want to make sure that the when the connection is closed, the loop end.
    event = asyncio.Event()
    await asyncpg_connector.open_async()
    async with asyncpg_connector._pool.acquire() as connection:
...
async def test_loop_notify_timeout(asyncpg_connector):
    ...
    async with asyncpg_connector._pool.acquire() as connection:
        task = asyncio.ensure_future(
            asyncpg_connector._loop_notify(
...
async def test_destructor(connection_params, capsys):
    connector = AsyncpgConnector(**connection_params)
    await connector.open_async()
